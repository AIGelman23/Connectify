...
export const resolvers = {
	Query: {
		...
		myConnections: async (parent, { currentUserId }, context) => {
			...
			if (!currentUserId) {
				console.warn("[BACKEND RESOLVER] No user ID provided for myConnections. Returning empty array.");
				return [];
			}
			...
			try {
				...
				console.log("[BACKEND RESOLVER] Final connections being returned (length):", finalConnections.length);
				return finalConnections;

			} catch (error) {
				console.error("[BACKEND RESOLVER] Error fetching myConnections:", error);
				return [];
			}
		},
	},
	...
	User: {
		...
		mutualFriends: async (parent, { targetUserId }) => {
			...
			const mutualFriendIds = Array.from(userAConnectedIds).filter(id => userBConnectedIds.has(id));
		...
		mutualFriendsCount: async (parent, { targetUserId }) => {
			...
			const mutualFriends = await resolvers.User.mutualFriends(parent, { targetUserId });
		...
		acceptConnectionRequest: async (parent, { requestId }) => {
			...
			await prisma.notification.create({
				data: {
					...
					message: `Your connection request to ${request.receiver.name || request.receiverId} was accepted!`,
		...
		rejectConnectionRequest: async (parent, { requestId }) => {
			...
			await prisma.notification.create({
				data: {
					...
					message: `Your connection request to ${request.receiver.name || request.receiverId} was rejected.`,
					senderId: request.receiverId,
					targetId: request.id,
				},
			});
	...
	Comment: {
		...
		parentComment: (parent) => {
			...
			return prisma.comment.findUnique({ where: { id: parent.parentCommentId } });
		},
		replies: (parent) => prisma.comment.findMany({ where: { parentCommentId: parent.id } }),
	},
	Notification: {
