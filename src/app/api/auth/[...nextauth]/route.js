		...
		CredentialsProvider({
			...
			async authorize(credentials) {
				...
				if (!user || !user.hashedPassword) {
					console.error(
						"Authorization failed: User not found or no hashed password for email:",
						credentials.email
					);
					// Return null to indicate authentication failure to NextAuth
					return null;
				}
				...
				if (isValidPassword) {
					...
					console.warn(
						"Authorization failed: Invalid password for email:",
						credentials.email
					);
			...
			if (user) {
				...
				if (dbUser) {
					...
					if (!token.jwt) {
						...
						token.jwt = `internal_fallback_jwt_for_user_${token.id
							}_${Date.now()}`;
					}
				}
			}
