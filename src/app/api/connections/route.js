...
export async function POST(request) {
  try {
    ...
    // Defensive: Ensure both users exist (fix duplicate receiver variable)
    ...
    // Defensive: Check for block table if it exists (optional)
    if (prisma.block) {
      ...
      if (block) {
        return NextResponse.json(
          {
            message: "Cannot connect: one of the users has blocked the other.",
          },
          { status: 403 }
        );
      }
    ...
    await prisma.notification.create({
      data: {
        ...
        type: "CONNECTION_REQUEST", // Must match your enum and database exactly
        message: `${
    ...
    console.error("API Error sending connection request:", error, error?.stack);
    return NextResponse.json(
      {
        message: error.message || "Internal server error sending request.",
        error: error.message,
      },
      { status: 500 }
    );
